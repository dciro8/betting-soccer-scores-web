...
  //Pide un método  
 ...
 imports: [
  ...
  fixture.detectChanges();
   ...
   component.getProductAll();
   
   expect(spy1).toHaveBeenCalled();
   
   expect(component.ProductDto?.length).toBe(0);
 ...
 it('GetAllSoocerTeam con Datos', () => {
  ...
  const soccerTeamRegister = fixture.debugElement.injector.get(GetSoccerTeamListComponent);
  
  
  // const spy1 = spyOn( soccerTeamRegister,'getProductAll').and.returnValue(of(listSoccerTeam));   
  
  //  component.getProductAll();
  
  // expect(spy1).toHaveBeenCalled();
  
  // console.log(' component.GetEstadoPacienteIngreso.length12', component.ProductDto?.length);
  
  // expect(component.ProductDto?.length).toBe(3);  
